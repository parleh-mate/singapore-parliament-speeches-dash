            ...
            dbc.Accordion(
                [
                    dbc.AccordionItem(
                        [
                            html.P(f"""
                                   Summaries are generated using a combination of a RAG model (a kind of search engine) and GPT. When a query is submitted, it is passed into the search engine and the top {top_k_rag} most similar entries from a database of pre-generated speech summaries are returned. These summaries are then passed into GPT to again summarize them into a coherent policy position. We use speech summaries rather than raw speeches since this tends to be quicker and more cost efficient, though possibly at the risk of some information loss. Please refer to the methodology section for more info.""")
                        ],
                        title=html.Span(
